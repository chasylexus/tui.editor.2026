...
에디터 사용 방식은 기존 v2.x와 동일하게 [스코프드 패키지(Scoped package)](https://docs.npmjs.com/using-npm/scope.html)를 적용하여 다음과 같이 `@techie_doubts/tui.editor.2026`로 패키지를 설치하여 사용한다. 아래는 npm 커맨드를 사용한 에디터 설치 예제이다.
...
```js
const Editor = require('@techie_doubts/tui.editor.2026'); /* CommonJS 방식 */
import Editor from '@techie_doubts/tui.editor.2026'; /* ES6 모듈 방식 */
```
...
```js
import { EditorCore } from '@techie_doubts/tui.editor.2026'; /* ES6 모듈 방식 */
...
두 번째로 다크 테마 지원을 위한 `theme/td-editor-dark.css` 파일이 추가되었다. 이에 대한 설명은 [다크 테마 추가](#-다크-테마-추가)에서 볼 수 있다.
...
#### 다크 테마 추가
v3.0에서는 다크 테마가 추가되었다. 다크 테마를 적용하고 싶은 경우 `theme/td-editor-dark.css`를 추가한 후, 에디터의 `theme` 옵션을 `dark`로 설정하여 사용한다. 현재 v3.0에서는 다크 테마만 지원하지만, 사용자가 여러 테마를 혼합하여 사용하거나 추후 다른 테마를 지원하기 위해 `theme` 옵션을 추가하였다.
...
v3.0부터는 jQuery Wrapper가 제거되었다. jQuery에서 사용을 원하는 경우 직접 `@techie_doubts/tui.editor.2026` 패키지를 랩핑하여 사용해야 한다.
