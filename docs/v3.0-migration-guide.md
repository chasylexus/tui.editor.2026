...
To use the Editor, use the [scoped package](https://docs.npmjs.com/using-npm/scope.html) to install the `@techie_doubts/tui.editor.2026` package as you did for the previous v2.x. The following is an example of using the npm command to install the Editor. 

```sh
$ npm install @techie_doubts/tui.editor.2026
$ npm install @techie_doubts/tui.editor.2026@<version>
```

#### Usages

```js
const Editor = require('@techie_doubts/tui.editor.2026'); /* CommonJS */
import Editor from '@techie_doubts/tui.editor.2026'; /* ES6 Module */
```
...
```js
import { EditorCore } from '@techie_doubts/tui.editor.2026'; /* ES6 Module */
...
Secondly, the `theme/td-editor-dark.css` is added for the dark theme support. The dark theme will be covered more in depth in [Added Dark Theme](#-added-dark-theme). 
...
#### Added Dark Theme
v3.0 ships with dark theme included. To apply the dark theme, add the `theme/td-editor-dark.css` and set the editor's `theme` option to be `dark`. Currently, in v3.0, only the dark theme is supported, but the `theme` option was added to support more diverse combinations of themes in the future. 
...
<head>
  ...
  />
  <link rel="stylesheet" href="https://uicdn.toast.com/editor/latest/td-editor.css" />
  ...
</head>
<body>
  ...
...
const popup = {
  name: 'color',
  tooltip: 'Text color',
  className: 'td-editor-toolbar-icons color',
  popup: {
    className: 'td-editor-popup-color',
...
From v3.0, the jQuery Wrapper has been removed. To use jQuery, the user must wrap the `@techie_doubts/tui.editor.2026` package separately.
